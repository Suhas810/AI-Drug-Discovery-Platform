...
# Import our new ML services
import ml_services

app = FastAPI(title="AI Drug Discovery API")

app.include_router(ml_services.router, prefix="/api/ml", tags=["ML Services"])
...
def get_dataset():
    # Load dataset
    try:
        csv_path = os.path.join(os.path.dirname(__file__), "..", "data", "combined_dataset.csv")
